...
class ServiceContainer {
    ...
    getApiClient() {
        if (!this.services.has('api')) {
            ...
            this.services.set('api', new api_client_service_1.ApiClientService(this.logger, config.baseUrl, config.timeout));
        }
    ...
    getOtpService() {
        if (!this.services.has('otp')) {
            ...
            const config = this.getConfigService().getApiConfig();
            this.services.set('otp', new otp_service_1.OtpService(this.logger, apiClient, config.baseUrl));
        }
    ...
    getGitHubService(token) {
        ...
        if (!this.services.has(serviceKey)) {
            this.services.set(serviceKey, new github_service_1.GitHubService(this.logger, token, github_1.context.repo.owner, github_1.context.repo.repo));
        }
