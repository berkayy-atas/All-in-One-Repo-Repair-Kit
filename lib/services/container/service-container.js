...
class ServiceContainer {
    ...
    getCryptoService() {
        if (!this.services.has('crypto')) {
            ...
            this.services.set('crypto', new crypto_service_1.CryptoService(this.logger, cryptoConfig));
        }
    ...
    getApiClient() {
        if (!this.services.has('api')) {
            ...
            this.services.set('api', new api_client_service_1.ApiClientService(this.logger, config));
        }
    ...
    getRestoreWorkflowService(patToken) {
        ...
        if (!this.services.has(serviceKey)) {
            this.services.set(serviceKey, new restore_workflow_service_1.RestoreWorkflowService(this.logger, this.getConfigService(), this.getCryptoService(), this.getCompressionService(), this.getGitService(), this.getApiClient(), this.getOtpService(), this.getGitHubService(patToken) || undefined));
        }
        return this.services.get(serviceKey);
    }
