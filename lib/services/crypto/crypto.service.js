...
class CryptoService extends base_service_1.BaseService {
    cryptoConfig;
    constructor(logger, cryptoConfig) {
        super(logger);
    ...
    async encrypt(inputBuffer, password) {
        this.ensureInitialized();
        try {
            const salt = (0, crypto_1.randomBytes)(this.cryptoConfig.saltLength);
            ...
            const iv = (0, crypto_1.randomBytes)(this.cryptoConfig.ivLength);
            const cipher = (0, crypto_1.createCipheriv)(this.cryptoConfig.algorithm, key, iv);
    ...
    async decrypt(encryptedBuffer, password) {
        ...
        try {
            ...
            const key = await pbkdf2Async(password, salt, this.cryptoConfig.iterations, this.cryptoConfig.keyLength, this.cryptoConfig.digest);
            const decipher = (0, crypto_1.createDecipheriv)(this.cryptoConfig.algorithm, key, iv);
