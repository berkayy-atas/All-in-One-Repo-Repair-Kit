...
### Understanding Endpoint Preferences

The `endpoint-preference` input allows you to control how your repositories are organized within iCredible File Security endpoints:

#### **SINGLE Mode (Default)**

- **Behavior:** Creates a separate, dedicated endpoint for each repository
- **Use Case:** Ideal for independent repository management and isolation
- **Example:**
  - Repository: `myorg/frontend` → Endpoint: `myorg/frontend`
  - Repository: `myorg/backend` → Endpoint: `myorg/backend`

#### **MULTIPLE Mode**

- **Behavior:** Groups all repositories under a single endpoint using the organization/owner name
- **Use Case:** Perfect for organizations managing multiple related repositories
- **Example:**
  - Repository: `myorg/frontend` → Endpoint: `myorg`
  - Repository: `myorg/backend` → Endpoint: `myorg`
  - Repository: `myorg/api` → Endpoint: `myorg`

### Configuration

Add the `endpoint-preference` input to your workflow file:

```yaml
with:
  icredible_activation_code: ${{ secrets.ICREDIBLE_ACTIVATION_CODE }}
  icredible_encryption_password: ${{ secrets.ICREDIBLE_ENCRYPTION_PASSWORD }}
  endpoint-preference: "SINGLE" # Options: 'SINGLE' or 'MULTIPLE' (default: 'SINGLE')
  action: "backup"
```

### Additional Resources

For detailed information about endpoint management, backup strategies, and advanced configuration options, please visit:
📚 **[iCredible File Security - Endpoint Management Documentation](https://dev.docs.file-security.icredible.com/docs/file-security/user-guide/endpoint-management#endpoint-backup)**
