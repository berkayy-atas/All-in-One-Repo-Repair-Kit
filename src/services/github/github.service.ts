...
export class GitHubService extends BaseService implements IGitHubService {
  ...
  protected async onInitialize(): Promise<void> {
    if (!this.octokit) {
      this.logger.warn(
        "No GitHub token provided - GitHub Actions suspension/resumption will not be available"
      );
    }
  }
    ...
    try {
      ...
      if (!suspensionState) {
        try {
          ...
          this.logger.warn(
            "Could not load suspension state from file, using default permissions"
          );
        }
      }
      ...
      try {
        ...
        this.logger.debug(
          "Could not remove actions permissions file (it may not exist)"
        );
      }
      ...
      this.logger.error(
        "Failed to resume GitHub Actions",
      ...
      this.logger.error(
        "All retry attempts failed - GitHub Actions may remain disabled"
      );
    }
  }
  ...
  public async getActionsPermissions(): Promise<GitHubActionsPermissions> {
    ...
    try {
      const response =
        await this.octokit.rest.actions.getGithubActionsPermissionsRepository({
          owner: this.owner,
          repo: this.repo,
        });
  ...
  public async setActionsPermissions(
    ...
    try {
      await this.octokit.rest.actions.setGithubActionsPermissionsRepository({
        ...
        allowed_actions: permissions.allowed_actions as
          | "all"
          | "local_only"
          | "selected"
          | undefined,
      ...
      this.logger.debug(
        `GitHub Actions permissions updated: enabled=${permissions.enabled}`
      );
    } catch (error) {
    ...
    try {
      ...
      const getOperatingSystem = (): "Linux" | "Windows" | "MacOS" => {
        ...
        switch (runnerOS) {
          ...
          default:
            this.logger.warn(
              `Unexpected operating system: ‘${runnerOS}’. ‘Linux’ is assumed.`
            );
            return "Linux";
        }
      };
