...
export class CryptoService extends BaseService implements ICryptoService {
  ...
  public async encrypt(inputBuffer: Buffer, password: string): Promise<Buffer> {
    ...
    try {
      ...
      const key = await pbkdf2Async(
        password,
        salt,
        this.cryptoConfig.iterations,
        this.cryptoConfig.keyLength,
        this.cryptoConfig.digest
      );
  ...
  public async decrypt(
    ...
    try {
      const salt = encryptedBuffer.subarray(
        8,
        8 + this.cryptoConfig.saltLength
      );
      const iv = encryptedBuffer.subarray(
        8 + this.cryptoConfig.saltLength,
        8 + this.cryptoConfig.saltLength + this.cryptoConfig.ivLength
      );
      const authTag = encryptedBuffer.subarray(
        8 + this.cryptoConfig.saltLength + this.cryptoConfig.ivLength,
        8 +
          this.cryptoConfig.saltLength +
          this.cryptoConfig.ivLength +
          this.cryptoConfig.authTagLength
      );
  ...
  public getEncryptedFileName(password: string): string {
    const repoName =
      process.env.GITHUB_REPOSITORY?.split("/").pop() || "repository";
