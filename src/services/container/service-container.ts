...
export class ServiceContainer {
  ...
  public getConfigService(): ConfigService {
    if (!this.services.has("config")) {
      ...
      this.services.set(
        "config",
        new ConfigService(this.logger, validationService)
      );
    }
  ...
  public getOtpService(): OtpService {
    if (!this.services.has("otp")) {
      ...
      this.services.set(
        "otp",
        new OtpService(this.logger, apiClient, config.managementBaseUrl)
      );
    }
  ...
  public getRestoreWorkflowService(patToken?: string): RestoreWorkflowService {
    const serviceKey = patToken
      ? `restore-workflow-${patToken.substring(0, 8)}`
      : "restore-workflow";

    if (!this.services.has(serviceKey)) {
      this.services.set(
        serviceKey,
        new RestoreWorkflowService(
          this.logger,
          this.getConfigService(),
          this.getCryptoService(),
          this.getCompressionService(),
