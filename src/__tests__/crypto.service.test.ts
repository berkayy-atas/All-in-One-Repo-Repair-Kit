...
describe('CryptoService', () => {
  ...
  describe('encryptWithIV and decryptWithIV', () => {
    const testData = Buffer.from('This is a test string for encryption with explicit IV');
    const password = 'SecurePassword123!';

    it('should encrypt and decrypt with IV successfully', async () => {
      const encrypted = await cryptoService.encryptWithIV(testData, password);
      const decrypted = await cryptoService.decryptWithIV(encrypted, password);
      
      expect(decrypted).toEqual(testData);
    });

    it('should produce different results than regular encrypt method', async () => {
      const encrypted1 = await cryptoService.encrypt(testData, password);
      const encrypted2 = await cryptoService.encryptWithIV(testData, password);
      
      // Different methods should produce different formats
      expect(encrypted1.length).not.toBe(encrypted2.length);
    });

    it('should have proper format with IV', async () => {
      const encrypted = await cryptoService.encryptWithIV(testData, password);
      ...
      // Should have minimum length (header + salt + IV + encrypted data)
      expect(encrypted.length).toBeGreaterThan(32); // 8 + 8 + 16 + data
    });
  });
  ...
  describe('error handling', () => {
    it('should handle invalid encrypted data format', async () => {
      ...
      ).rejects.toThrow('Invalid encrypted data header');
    });
